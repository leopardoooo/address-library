...
//			Alert('IE?',Ext.browser.userAgent + '    ' + Ext.browser.engineName);
			...
			Addr.queryResultGrid.contextMenu.showAt(event.getXY());
			Addr.contextMenuHandler.filterMenuItems(Addr.queryResultGrid.contextMenu,record);
			return false;
		}
	},
        ...
        items:[
               {
		        ...
		        {
					text : '新增地址',needFielterByFgs:true, glyph : 0xf0c7,addChildren:true,
					handler : function() {
						var rec = Addr.queryResultGrid.selModel.getSelection();
		            	if(rec.length != 1){
		            		Alert('提示','请选择且仅选择一条记录！');
	...
	columns: [
	    ...
	    ,{text: '备注',flex: 1,dataIndex: 'addrUse',sortable: true ,align:'center',renderer:Addr.util.qtipValue}
	]
});
